...
Soll ein User Administrator sein, so muss manuell in der Datenbank die Zelle "is_admin" auf 1 gesetzt werden
*/
...
if(isset($_POST['bt_create_user']))
{
    ...
    if($password == NULL || strlen($password) < 6){
        $errors[] = 'Passwort mind. 6 Zeichen!';
    }

    // Prüfen ob es bereits einen User mit dieser E-Mail gibt
    if($userService->getUserByEmail($email) !== FALSE){
        $errors[] = 'Es existiert bereits ein User mit dieser E-Mail Adresse!';
    }

    // Prüfen ob es Fehler bei der Validierung gab
    if(count($errors) == 0)
    {
