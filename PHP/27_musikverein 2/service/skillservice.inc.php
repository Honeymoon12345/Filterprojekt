...
class SkillService 
{
    ...
    // Auswertung: Welches Instrument (Kenntnis) wird von den meisten Mitgliedern beherrscht? 
    // Gibt ein Objekt der Klasse Skill zurück, oder false wenn kein Mitglied einen zugewiesenen Skill hat.
    public function getSkillWithMaxUsers() : Skill|false 
    {
        // Ermittle für jeden Skill wie viele Mitarbeiter diesen beherrschen
        // und sortiere das Ergebnis absteigend (Skill mit meisten Usern zuerst)
        $ps = $this->conn->prepare('
            SELECT s.id AS skillId, s.name AS skillName, COUNT(s.id) AS userCountPerSkill
            FROM skill s 
            INNER JOIN user_skill us ON (s.id = us.skill_id)
            GROUP BY s.id
            ORDER BY userCountPerSkill DESC
        ');
        $ps->execute();

        while($row = $ps->fetch()){
            // Da absteigend sortiert, ist das Ergebnis der erste Datensatz
            return new Skill($row['skillId'], $row['skillName']);
        }

        // Noch hat niemand einen zugewiesenen Skill 
        return false;
    }


    // Ermittelt für jeden Skill wie viele Mitglieder diesen Skill beherrschen
    // Gibt ein Array von Objekten der Klasse SkillUserCounter zurück
    public function getUsersPerSkill() : array 
    {
        // Ermittle für jeden Skill wie viele Mitarbeiter diesen beherrschen
        // und sortiere das Ergebnis absteigend (Skill mit meisten Usern zuerst)
        // Die Klasse wurde nach der Klasse SkillService definiert 
        $ps = $this->conn->prepare('
            SELECT s.id AS skillId, s.name AS skillName, COUNT(s.id) AS userCountPerSkill
            FROM skill s 
            INNER JOIN user_skill us ON (s.id = us.skill_id)
        ...
        while($row = $ps->fetch()){
            ...
            $userCount = $row['userCountPerSkill'];

            $result[] = new SkillUserCounter($skill, $userCount);
        }
        return $result; 
...
// für die Auswertung
class SkillUserCounter {
    public Skill $skill;
    public int $userCount;
    public function __construct(Skill $skill, int $userCount)
    {
        $this->skill = $skill;
